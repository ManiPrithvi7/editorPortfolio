...
const Sidebar = ({ isOpen, onNavigate }: SidebarProps) => {
  ...
  const navItems = [
    { path: "/", icon: <Home size={20} />, label: "Home", fileName: "myIntro.jsx" },
  ...
  return (
    <aside 
      className={`fixed bg-vscode-sidebar fixed inset-y-0 left-0 z-40 transform transition-all duration-300 ease-in-out
    ...
    >
      <div className="h-full flex flex-col">
        ...
        <div className="px-4 py-2 text-vscode-text/80 text-sm">
          <div className={`transition-opacity duration-200 ${!isExtended && "md:opacity-0"} py-3 px-2`}>
            PORTFOLIO
          </div>
        </div>
